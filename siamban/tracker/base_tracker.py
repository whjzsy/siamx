...
class SiameseTracker(BaseTracker):
    def get_subwindow(self, img, center_pos, model_size, original_size,
                      avg_chans):
        """ Todo ??? 此部分运行较为缓慢, 有待改进
        采用center_pos周围的子窗口
        Args:
            img(np.ndarray): BGR based image
            center_pos(np.ndarray): center position
            model_size(): exemplar size
            original_size(): original size, 加了缩放因数CONTEXT_AMOUNT的大小
