...
parser.add_argument('--window-influence', default='0.1,0.8,0.05',
                    type=parse_range)
parser.add_argument('--search-region', default='255,256,8',
                    type=parse_range_int)
...
def run_tracker(tracker, img, gt, video_name, restart=True):
    ...
    if restart:  # VOT2016 and VOT 2018
        ...
        print(
            'Video: {:12s} Time: {:4.1f}s Speed: {:3.1f}fps Lost: {:d}'
                .format(video_name, toc, idx / toc, lost_number)
        )
        return pred_bboxes
    else:
        toc = 0
...
if __name__ == '__main__':
    ...
    benchmark_path = os.path.join('hp_search_results', model_name, args.dataset)
                    ...
                    for ins in args.search_region:
                        ...
                        tracker_path = os.path.join(benchmark_path,
                                                    (model_name +
                                                     ...
                                                     '_wi-{:.3f}'.format(wi) +
                                                     '_lr-{:.3f}'.format(lr)))
                        if 'VOT2016' == args.dataset \
                            or 'VOT2018' == args.dataset \
                            or 'VOT2019' == args.dataset:

                            video_path = os.path.join(tracker_path, 'baseline',
                                                      video.name)
                            ...
                            with open(result_path, 'w') as f:
                                for x in pred_bboxes:
                                    if isinstance(x, int):
                                        ...
                                        f.write(','.join(
                                            [vot_float2str("%.4f", i) for i in
                                             x]) + '\n')

                        elif 'VOT2018-LT' == args.dataset:
                            video_path = os.path.join(tracker_path, 'longterm',
                                                      video.name)
                            result_path = os.path.join(video_path,
                                                       '{}_001.txt'.format(
                                                           video.name))
                            if _check_and_occupation(video_path, result_path):
                                continue
                            pred_bboxes, scores, track_times = run_tracker(
                                tracker,
                            ...
                            video_path = os.path.join('epoch_result',
                                                      tracker_path, video.name)
                            if not os.path.isdir(video_path):
                                os.makedirs(video_path)
                            result_path = os.path.join(video_path,
                                                       '{}_001.txt'.format(
